...
select hasnt_function('public', 'delete_own_account', array[]::name[],
  'delete_own_account() is dropped — account deletion is POST /account/delete (20260911000000)');
...
-- may call the user-facing RPCs. delete_own_account() no longer exists
-- (20260911000000_account_delete_hardening.sql dropped it — account
-- deletion goes through POST /account/delete, apps/api/src/account.ts,
-- which cancels Stripe billing and removes Storage objects first). The negative
...
-- unauthenticated (export_my_data_test.sql).
-- --------------------------------------------------------------------------
