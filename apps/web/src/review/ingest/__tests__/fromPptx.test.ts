...
describe('fromPptx', () => {
  ...
  it('removes the raw upload and preserves the error when signing fails', async () => {
    const signingError = { message: 'signing unavailable' };
    mockCreateSignedUrl.mockResolvedValue({ data: null, error: signingError });
