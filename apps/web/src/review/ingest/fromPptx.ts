...
export async function fromPptx(
  ...
  try {
    ...
    // Best-effort: remove the raw .pptx after any successful upload
    // (success path + signing/render failures). RLS protects against
    // cross-user deletes; a removal failure is fine.
    if (uploaded) {
