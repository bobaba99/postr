...
10. **Gallery moderation pre-publish approval queue**: The shipped v1 moderation is _post-hoc_ — moderators in the `admin_emails` allowlist can force-retract content from `/admin/gallery` after it goes live. A more mature moderation pipeline would add a _pre-publish_ approval queue (pending/approved/rejected states, reviewer UI, author notification on decision) so sensitive content is gated before becoming public. Worth doing once the gallery passes a few hundred entries or starts drawing legal risk.
