...
flowchart LR
  ...
  PS -->|"refund terms (fine print)"| TRM["/terms#refunds"]
...
- [ ] `refund terms` — Link — `/terms#refunds` (`REFUND_TERMS_PATH`) — section fine print — `PricingSection.tsx:160`
...
- [ ] "A term is refundable in full within 14 days of a charge, a pack until its first export; taking a paid export ends either refund. Full details in the refund terms." — section fine print under the grid ("refund terms" links `/terms#refunds`) — `PricingSection.tsx:156-160` (2026-09-11)
...
#### `data/refundCopy.ts` — pre-purchase refund lines — no UI, constants only (2026-09-11)

**Copy** (rendered by `PricingSection`, `EditableExportButtons`, `Auth` — see the §6.3 "Refund rule in front of the buyer" bullet)
- [ ] "Full refund within 14 days of a charge if you haven’t taken a paid export." — `REFUND_LINE.term`
- [ ] "Full refund until your first export. No refund after, even in part." — `REFUND_LINE.pack`
- [ ] "Term: full refund within 14 days of a charge if you haven’t taken a paid export. Pack: full refund until your first export, none after — even in part." — `REFUND_LINE_BOTH`
- [ ] `/terms#refunds` — `REFUND_TERMS_PATH`
...
| `POST /billing/create-checkout` | `data/billing.ts:17` ← EditableExportButtons (term/pack), Auth (`startCheckoutForPlan`) | Creates Stripe Checkout Session → redirect URL No `custom_text` / `consent_collection` on the session — Stripe refuses `custom_text` with Managed Payments (sandbox 2026-09-11); the refund rule is shown client-side BEFORE checkout (§6.3) |
...
| `POST /billing/refund` | `data/billing.ts` ← Profile `SubscriptionPanel` (`requestRefund('term'|'pack')`) | Term (within 14 days, no paid export since the charge) / pack (the most recent unrefunded pack in FULL, only while no credit has been used — `billing/packRefund.ts`; one export → `409 already_used`) refunds; a term refund cancels the Stripe subscription immediately and answers `subscription_cancelled: true` (P0-1, 2026-09-11) |
...
## 12. Brand mark, watermarks & plot code export (added 2026-08-06, branch `feat/plot-branding-audit-flow`)

### 12.1 Canonical logo geometry (single source)

- [ ] `apps/web/src/brand/markGeometry.ts` is the ONE source for the mark's geometry (two crossing curves + centre dot, true 40×40 square in a 64 viewBox, dot `r=6` so it caps the crossing). Exports path strings, colours, `markInnerSvg(tone)`, `markSvg(size, opts)`. Golden-guard tests in `brand/__tests__/markGeometry.test.ts`.
- [ ] Every prior copy of the rectangular `M14 14…` geometry was replaced with the square `M12 52…` paths: `public/favicon.svg`, `brand/icon-rounded.svg`, `brand/icon-square.svg`, `components/PublicHeader.tsx`, `components/PublicFooter.tsx`, `pages/{Auth,Home,AdminGallery,Profile}.tsx`, `poster/Sidebar.tsx`, `export/ackMark.ts`, `scripts/rasterize-brand.mjs`. **If the geometry changes, change `markGeometry.ts` AND the duplicated constants in `rasterize-brand.mjs` (a .mjs can't import the .ts).**
- [ ] `export/ackMark.ts` uses the new geometry but stays MONOCHROME muted grey (`#6b7280`) — the anti-"vendor sticker" acknowledgement rendition. Do NOT colour it.

### 12.2 Logo library

- [ ] `apps/web/brand/library/` — generated by `scripts/rasterize-brand.mjs` (manual dev script, NOT in `npm run build`). Variants: transparent / white-bg / purple-bg / mono-dark / mono-light / muted, each SVG + PNG at 32–1024; plus mark+wordmark lockups. `index.json` + `README.md` manifest. Use SVG where it renders; PNG where SVG breaks a PDF pipeline.

### 12.3 Poster/slide PDF colophon watermark (settled 2026-08-06 — HARD RULE)

- [ ] The print/PDF colophon and the PPTX colophon now carry a SMALL MUTED PNG logo beside "Poster made with postr.sh". PNG not inline SVG (some PDF engines mis-render inline SVG in the print window). Small, muted, in the bottom-margin overlay — never overlaps content. Settled partial change to the 2026-07-27 "no logo" rule; a coloured band is still forbidden.
- [ ] `export/colophonMarkPng.ts` — the muted mark as a frozen PNG data URI (from `brand/library/postr-mark--muted@128.png`). Consumed by `export/attribution.ts` (`acknowledgementPrintHtml/Css`) and `export/pptx/writer.ts`. Still gated by `shouldAttribute()`. **PDF paid-seam quirk unchanged** (manual-test-flows §22: editor PDF hardcodes `attribution:{}` → always attributes).

### 12.4 Plot-picker credit on chart exports

- [ ] `charts/plotCredit.ts` — "made by postr.sh" + the square logo, 11px flush-right purple, added ONLY at the download seam in `charts/download.ts` `chartToSvgString` (NOT in `renderChart`, so charts inserted into a poster aren't double-marked; on-screen previews stay clean). Lives in added canvas below the plot — plot geometry untouched. Covers SVG/PNG/ZIP. Gated by `shouldAttribute()`.

### 12.5 Plot-picker R / Python code export
