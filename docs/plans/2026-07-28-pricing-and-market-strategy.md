...
a surprise. Checkout gets wired when the paywall is built. Provider:
**Stripe Managed Payments** (a Merchant of Record — chosen for maturity,
compliance, and MCP over the cheaper Lemon Squeezy / Polar).

**All three tiers sell live products (MODEL CORRECTED 2026-07-28).** An
earlier revision made the pack a coming-soon "talk" tier with a waitlist.
That is superseded: the paywall gates **all editable exports** — PPTX +
LaTeX for posters, which SHIP TODAY — so the **$9.99 pack sells 3 real
poster exports now.** The term is unlimited exports. When paper-to-talk
ships, talk export simply joins the same paywall (spends one pack credit,
or is unlimited on the term) — no pricing change, no waitlist, no
free-deck promise to honour. The editor and all tools stay free.
