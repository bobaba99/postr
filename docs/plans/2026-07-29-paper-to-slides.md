...
- **Design architecture (DECIDED 2026-07-29 by experiment — see
  `experiments/design-pass-architecture.md`): Arm T + Arm P.** Arm T = a
  field-aware **theme** (palette + type scale + layout rules) generated by a text
  model and applied deterministically (the parameter layer, ~$0.007/deck). Arm P
  = a **one-step structured styled deck** — GPT emits per-slide styled layout as
  *data* (not an image), so every slide stays editable text (~$0.016/deck, judged
  clean by Gavin). No image-generation dependency in current scope.
  **Deferred to a future design feature:** Arm I (generated decorative imagery +
  the T-recolors-I composability). Claude Design MCP stays out (headless-
  unavailable in a server path).
