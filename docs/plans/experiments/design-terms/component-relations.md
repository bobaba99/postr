...
slide's components, so a vague spatial edit ("move the text lower, into the
center", "these two are too close") resolves to concrete component operations —
**looked up here, then combined with the crosswalk lookup + few-shot.**

> **The flow this enables:** user says *"move the text lower, in the center"* →
...
- **"these two are too close"** → identify the adjacent pair in the stack, then
...
## Worked examples (how a request resolves)

**"move the text lower, in the center"**
1. "the text" → dominant text = `title` (currently top-left @ y≈1.4).
2. "lower" → increase y toward middle/bottom band; "in the center" → set x to
   center band + horizontal-align center.
3. Check relations: `title` has a following `body`/`quote-block` in the stack —
   moving the title down reduces its gap to the body → **push the body down too**
   (or the user meant the whole content block; ambiguous edge → the "ask first"
   path if it can't preserve the stack cleanly).
4. Apply: title → middle-center, aligned center; body follows to keep the gap.

**"the headline and the quote are too close"**
1. Identify the adjacent stack pair: `title` → `quote-block`.
2. Op: increase the vertical gap between exactly those two (whitespace crosswalk
   op, scoped to the pair). Everything below the quote shifts down or the slide
   redistributes.

**"move the whole thing up, too much empty space at the top"**
1. Not one component — the **content stack** (section-label → title → body).
2. Op: reduce the top margin / shift the stack up as a group, preserving internal
   gaps and the alignment group's left edge.
