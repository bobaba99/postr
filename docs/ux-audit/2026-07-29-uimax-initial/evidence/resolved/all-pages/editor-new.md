# New poster editor — UIMax full review

- Audited URL: `http://127.0.0.1:4174/p/new`
- Generated: 2026-07-29
- Viewport: 1440 × 900
- Status: resolved re-audit

# UIMax Data Collection Complete

**URL:** http://127.0.0.1:4174/p/new
**Code Directory:** /Users/zihaogeng/development/postr-ux-audit-flow-map/apps/web/src
**Timestamp:** 2026-07-30T03:48:29.352Z
**Accessibility violations:** 0
**Accessibility passes:** 15
**Load time:** 76ms
**DOM nodes:** 45
**Code files analyzed:** 200
**Code findings:** 1547
**Framework detected:** react

## Report Card
| Section | Grade |
|---------|-------|
| Accessibility | **A+** (100) |
| Performance | **A-** (91) |
| Best Practices | **A** (96) |
| SEO | **D** (95) |
| Code Quality | **F** (0) |

**Lighthouse Performance:** 91
**Lighthouse Accessibility:** 100
**Lighthouse Best Practices:** 96
**Lighthouse SEO:** 66
**SEO Score:** 95/100 (15 passed, 3 failed)

---

## Screenshot of the live UI — study this carefully:


---

# UIMax Review Report

**URL:** http://127.0.0.1:4174/p/new
**Code Directory:** /Users/zihaogeng/development/postr-ux-audit-flow-map/apps/web/src
**Generated:** 2026-07-30T03:48:29.352Z

## Report Card

| Section | Grade | Score | Rating |
|---------|-------|-------|--------|
| Accessibility | **A+** | 100/100 | Exceptional |
| Performance | **A-** | 91/100 | Great |
| Best Practices | **A** | 96/100 | Excellent |
| SEO | **D** | 95/100 | Weak |
| Code Quality | **F** | 0/100 | Failing |


---

## Accessibility Audit Results

**URL:** http://127.0.0.1:4174/p/new
**Scanned:** 2026-07-30T03:48:20.814Z
**Violations:** 0
**Passes:** 15
**Incomplete:** 0

No accessibility violations found.

---

## Performance Metrics
**URL:** http://127.0.0.1:4174/p/new
**Measured:** 2026-07-30T03:48:23.250Z
### Core Web Vitals
| Metric | Value | Rating |
|--------|-------|--------|
| First Contentful Paint (FCP) | 696ms (Good) |
| Largest Contentful Paint (LCP) | 696ms (Good) |
| Cumulative Layout Shift (CLS) | 0.000 (Good) |
| Total Blocking Time (TBT) | 0ms (Good) |
### Page Metrics
- **Load Time:** 76ms
- **DOM Content Loaded:** 76ms
- **DOM Nodes:** 45
- **Resources:** 15
- **Total Transfer Size:** 4.1 KB
- **JS Heap Size:** 6.3 MB

---

## Lighthouse Audit

**URL:** http://127.0.0.1:4174/p/new
**Lighthouse Version:** 13.4.1
**Measured:** 2026-07-30T03:48:21.335Z

### Scores
| Category | Score |
|----------|-------|
| Performance | 91 (Good) |
| Accessibility | 100 (Good) |
| Best Practices | 96 (Good) |
| SEO | 66 (Needs Improvement) |

### Key Findings (Needs Attention)
| Status | Audit |
|--------|-------|
| FAIL | Largest Contentful Paint — 3.5 s |
| FAIL | Reduce unused JavaScript — Est savings of 294 KiB |

### Passing Audits
| Status | Audit |
|--------|-------|
| PASS | First Contentful Paint — 1.5 s |
| PASS | Total Blocking Time — 0 ms |
| PASS | Cumulative Layout Shift — 0 |
| PASS | Speed Index — 1.5 s |
| PASS | Time to Interactive — 3.5 s |
| PASS | Minify JavaScript |
| PASS | Minify CSS |
| PASS | Reduce unused CSS |
| PASS | JavaScript execution time — 0.2 s |
| PASS | Minimizes main-thread work — 0.5 s |
| PASS | Initial server response time was short — Root document took 0 ms |
| PASS | Avoid multiple page redirects |
| PASS | Document has a `<title>` element |
| PASS | Document has a meta description |
| PASS | Page has successful HTTP status code |
| PASS | Links have a discernible name |
| PASS | Background and foreground colors have a sufficient contrast ratio |


---

## SEO Audit Results

**URL:** http://127.0.0.1:4174/p/new
**Score:** 95/100
**Passed:** 15 | **Failed:** 3

### Issues Found

#### LOW
- **Meta Robots**: Page has "noindex" in meta robots — search engines will NOT index this page. Remove if this is unintentional.
  Current: noindex,nofollow
- **Title Length**: Title is too short (21 chars). Aim for 30-60 characters for optimal search display.
  Current: 21 characters
- **Description Length**: Description is too short (24 chars). Aim for 120-160 characters for optimal search display.
  Current: 24 characters

### Passing Checks

- Page Title (Poster editor | Postr)
- Meta Description (The Postr poster editor.)
- Single H1 Tag (1 H1 tag(s))
- Heading Hierarchy (1 headings (proper hierarchy))
- Canonical URL (https://www.postr.sh/p)
- HTML Lang Attribute (en)
- Viewport Meta Tag (width=device-width, initial-scale=1.0)
- Image Alt Text (No images found)
- Open Graph Title (Poster editor | Postr)
- Open Graph Description (The Postr poster editor.)
- Open Graph Image (https://www.postr.sh/og-card.png)
- Twitter Card Meta (summary_large_image)
- Structured Data (JSON-LD) (1 block(s) found)
- Character Encoding (UTF-8)
- Link Accessibility (No links found)


---


## Repository-wide code scan

UIMax also ran its repository-wide heuristic scan. That repeated output is omitted here because it is identical across route reports and is not page-specific; the audit summary preserves its scope and caveat.
